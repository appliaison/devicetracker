...
if (strlen($_POST['btn_submit']) > 0)
{
		$handheld_id = $_POST['handheld_id'];
		$hh_type = $_POST['hh_type'];
		$hh_location = $_POST['hh_location'];
		$hh_assigned_num = $_POST['hh_assigned_num'];
		$hh_revision_num = $_POST['hh_revision_num'];
		$hh_pin = $_POST['hh_pin'];
		$hh_imei = $_POST['hh_imei'];
		$hh_bootrom = $_POST['hh_bootrom'];
		$hh_cpr = $_POST['hh_cpr'];
		$hh_notes = $_POST['hh_notes'];
		$xhh_is_secure = $_POST['xhh_is_secure'];
		$query = $_POST['query'];
		if (strlen($xhh_is_secure)>0)
		{
			$xhh_is_secure = 1;
		}
		else
		...
		$xhh_is_corp = $POST['xhh_is_corp'];
		if (strlen($xhh_is_corp)>0)
		{
			$xhh_is_corp = 1;
		}
		else
		...
		if ($num_rows==1)
		{
		 ...
		 $sqlupdate = "UPDATE devicetracker.handhelds SET type = '$hh_type', loc_id = $hh_location, AssignedNumber = $hh_assigned_num, RevisionNumber = '$hh_revision_num', PIN = '$hh_pin', "
					. " IMEI = '$hh_imei', bootrom = '$hh_bootrom', CPR = '$hh_cpr', userid=$hh_userid, Notes = '$hh_notes', is_secure = $xhh_is_secure, is_corp = $xhh_is_corp WHERE "
					. " Id = $handheld_id ";
...
else
{
	...
	<tr>
		<th>
		<input type="checkbox" name="xhh_is_secure" id="xhh_is_secure"  class="checkbox" <? if ($hh_is_secure) echo " checked "; ?> />
		<label>Secure</label>
		<input type="checkbox" name="xhh_is_corp" id="xhh_is_corp" class="checkbox"<? if ($hh_is_corp) echo " checked "; ?>/>
